...
echo "==> systemd-сервис"
sudo cp deploy/oracle/vacancy-bot.service /etc/systemd/system/${SERVICE_NAME}.service
sudo sed -i "s|/home/ubuntu/vacancy-bot|${APP_DIR}|g" /etc/systemd/system/${SERVICE_NAME}.service
sudo sed -i "s|User=ubuntu|User=${USER}|g" /etc/systemd/system/${SERVICE_NAME}.service
sudo sed -i "s|Group=ubuntu|Group=${USER}|g" /etc/systemd/system/${SERVICE_NAME}.service
